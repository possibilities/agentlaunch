...
reports those decisions as labelled rows — a label, then facts, never
prose, so the output stays scannable as launch features accumulate. That narration is commentary, not the result:
