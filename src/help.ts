...
Commands:
  ...
  --x-surface  One-screen interactive launch form: the same launch, spoken
               to a surface instead of executed. Renders on stderr and
               writes one session-directive JSON line to stdout per submit,
               for a surface host (agentsurface) to realize as a herdr
               session. Takes no other arguments.
...
Takes no other arguments. Needs a terminal on stdin and stderr, and refuses
